...
const SignUpPage = () => {
  ...
  return (
      <div className='min-h-screen grid lg:grid-cols-2'>
              ...
              <div className='w-full max-w-md space-y-8'>
                  <div className='text-center mb-8'>
                      <div className='flex flex-col items-center gap-2 group'>
                          ...
                          <p className='text-base-content'>get started with your free account</p>
                      </div>
                  </div>  
                  <form onSubmit={handleSubmit} className='space-y-6'>
                      <div className='form-control'>
                          ...
                          <div className='relative'>
                              ...
                              placeholder='person name' value={formData.fullName} onChange={(e)=>setFormData({...formData, fullName:e.target.value})}></input>
                          </div>
                      </div>
                      <div className='form-control'>
                          <label className='label'>
                      ...
                      <div className='form-control'>
                          ...
                          <div className='relative'>
                              ...
                              <input type={showPassword ? 'text':'password'} className={`input input-bordered w-full pl-10`}
                                  placeholder='****' value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })}></input>
                              <button type='button' className='absolute inset-y-0 right-0 pr-3 flex items-center'
                                  onClick={() => setShowPassword(!showPassword)}>
                                  {showPassword ? (
                                  <EyeOff className='size-5 text-base-content/40'></EyeOff>
                                  ):(<Eye className='size-5 text-basec-content/40'></Eye>)}
                              </button>
                          </div>
                      </div>
                      <button type='submit' className='btn btn-primary w-full' disabled={isSigningUp}>
                          {isSigningUp ?(<><Loader2 className='size-5 animate-spin'></Loader2>loading</>):('create account')}
                      </button>
                  </form>
          ...
          <AuthImagePattern title='join our community' text='connect with friends share moments and stay in touch with your loved ones'></AuthImagePattern>
      </div>
  )
}
